...
namespace CpuUtil
{
	...
	bool StartCpuTracing(CpuUsageInfo* cpuUsage)
	{
		...
		BOOL retVal = GetProcessTimes(GetCurrentProcess(), &creationTimeDummy, &exitTimeDummy, &cpuUsage->kernelStartTime, &cpuUsage->userStartTime);
	...
	bool StopCpuTracing(CpuUsageInfo* cpuUsage)
	{
		...
		BOOL retVal = GetProcessTimes(GetCurrentProcess(), &creationTimeDummy, &exitTimeDummy, &(cpuUsage->kernelStopTime), &(cpuUsage->userStopTime));
	...
	std::string GetCpuStats(CpuUsageInfo* cpuUsage)
	{
		...
		double cpuPercentage = static_cast<double>((kernelStopTimeInt.QuadPart - kernelStartTimeInt.QuadPart + userStopTimeInt.QuadPart - userStartTimeInt.QuadPart)) / static_cast<double>(trackingStopTimeInt.QuadPart - trackingStartTimeInt.QuadPart) / static_cast<double>(sysInfo.dwNumberOfProcessors) * 100.0;
		...
		cpuUsage->elapsedTime = static_cast<double>(trackingStopTimeInt.QuadPart - trackingStartTimeInt.QuadPart) / 10000000;

		//Total Kernel Time in milliseconds:
		cpuUsage->kernelTimeMilliseconds =
			((double)kernelSystemTime.wHour) * 60 * 60 * 1000
			+ ((double)kernelSystemTime.wMinute) * 60 * 1000
			+ ((double)kernelSystemTime.wSecond) * 1000;

		//Total User Time in milliseconds:
		cpuUsage->userTimeMilliseconds =
			((double)kernelSystemTime.wHour) * 60 * 60 * 1000
			+ ((double)kernelSystemTime.wMinute) * 60 * 1000
			+ ((double)kernelSystemTime.wSecond) * 1000;

		std::ostringstream ss;
		ss << "Kernel Time: " << kernelSystemTime.wHour << ":" << kernelSystemTime.wMinute << ":" << kernelSystemTime.wSecond << "." << kernelSystemTime.wMilliseconds << "\n";
		ss << "User Time: " << userSystemTime.wHour << ":" << userSystemTime.wMinute << ":" << userSystemTime.wSecond << "." << userSystemTime.wMilliseconds << "\n";
