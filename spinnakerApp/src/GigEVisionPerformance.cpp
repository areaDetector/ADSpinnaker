...
void PrintUsage()
{
	...
	cout << argBayerRG << "		<Optional. Sets Pixel Format to BayerRG 8 or BayerRG16 for color cameras, using 8 or 16 as argument>" << endl;
...
bool ParseArguments(int argc, char* argv[])
{
	...
	for (int argument = 1; argument < argc; ++argument)
	{
		...
		if (strncmp(argv[argument], argUserSetFrames, strlen(argUserSetFrames)) == 0)
		{
			if (argument + 1 <= argc)
			{
				UserSetFramerate = atof(argv[argument + 1]);
				argument++;
			}
		}
...
void PrintDataStreamInfo(const Spinnaker::CameraPtr pCamera)
{
	try
	{
		...
		if (camInfo.StreamID != NULL) outSS << "Stream ID: " << camInfo.StreamID.ToString() << endl;
		if (camInfo.StreamType != NULL) outSS << "Stream Type: " << camInfo.StreamType.ToString() << endl;
		if (camInfo.StreamDefaultBufferCount != NULL)outSS << "Stream Buffer Count: " << camInfo.StreamDefaultBufferCount.ToString() << endl;
		if (camInfo.StreamBufferHandlingMode != NULL) outSS << "Stream Buffer Handling Mode: " << camInfo.StreamBufferHandlingMode.ToString() << endl;

		if (camInfo.GevTotalPacketCount != NULL)  outSS << "Stream Packets Received: " << camInfo.GevTotalPacketCount.ToString() << endl;
		if (camInfo.GevFailedPacketCount != NULL)  outSS << "Stream Packets Missed: " << camInfo.GevFailedPacketCount.ToString() << endl;
		if (camInfo.GevResendPacketCount != NULL)  outSS << "Stream Retransmitted Packets: " << camInfo.GevResendPacketCount.ToString() << endl;
		if (camInfo.StreamFailedBufferCount != NULL)  outSS << "Stream Failed Buffer Count: " << camInfo.StreamFailedBufferCount.ToString() << endl;
		if (camInfo.StreamBufferUnderrunCount != NULL)  outSS << "Stream Buffer Underrun Count: " << camInfo.StreamBufferUnderrunCount.ToString() << endl;
		if (camInfo.GevResendRequestCount != NULL)   outSS << "Stream Retransmitted Packets Requested: " << camInfo.GevResendRequestCount.ToString() << endl;
...
void PrintCPUUsage()
{
	...
	stringstream outSS;
	outSS << "Kernel Time: " << cpuUsageInfo.kernelSystemTime.wHour << "H" << ":" << cpuUsageInfo.kernelSystemTime.wMinute << "M" << ":" << cpuUsageInfo.kernelSystemTime.wSecond << "S" << ":" << cpuUsageInfo.kernelSystemTime.wMilliseconds << "ms" << "\n";
	outSS << "User Time: " << cpuUsageInfo.userSystemTime.wHour << "H" << ":" << cpuUsageInfo.userSystemTime.wMinute << "M" << ":" << cpuUsageInfo.userSystemTime.wSecond << "S" << ":" << cpuUsageInfo.userSystemTime.wMilliseconds << "ms" << "\n";

	outSS << "Kernel Time: " << ((cpuUsageInfo.kernelSystemTime.wHour * 60 * 60 * 1000) + (cpuUsageInfo.kernelSystemTime.wMinute * 60 * 1000) + (cpuUsageInfo.kernelSystemTime.wSecond * 1000) + (cpuUsageInfo.kernelSystemTime.wMilliseconds)) << "ms" << "\n";
	outSS << "User Time: " << ((cpuUsageInfo.userSystemTime.wHour * 60 * 60 * 1000) + (cpuUsageInfo.userSystemTime.wMinute * 60 * 1000) + (cpuUsageInfo.userSystemTime.wSecond * 1000) + (cpuUsageInfo.userSystemTime.wMilliseconds)) << "ms" << "\n";
...
bool EnableManualFramerate(CameraPtr pCam)
{
	...
	// Retrieve integer value from entry node
	int valueFrameRateAutoOff = ptrFrameRateAutoModeOff->GetValue();
...
int RunSingleCamera(CameraPtr pCam)
{
	...
	try
	{
		...
		try
		{
			...
			float minExpoTime = ExposureTimeNode->GetMin();		
			ExposureTimeNode->SetValue((minExpoTime + 10));
		}
		catch (Spinnaker::Exception &e)
		{
