        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            try
            {
                ...
                for (int loopCnt = 0; loopCnt < k_numLoops; loopCnt++)
                {
                    ...
                    CIntegerPtr pInt;
                    sNodeMap.InvalidateNodes();
                    pInt = sNodeMap.GetNode("StreamDeliveredFrameCount");
                    if (!IsAvailable(pInt) || !IsReadable(pInt))
                    {
                        cout << "Unable to read StreamDeliveredFrameCount. Aborting..." << endl << endl;
                        return -1;
                    }
                    cout << "StreamDeliveredFrameCount:" << pInt->GetValue() << endl;

                    pInt = sNodeMap.GetNode("StreamLostFrameCount");
                    if (!IsAvailable(pInt) || !IsReadable(pInt))
                    {
                        cout << "Unable to read StreamLostFrameCount. Aborting..." << endl << endl;
                        return -1;
                    }
                    cout << "StreamLostFrameCount:" << pInt->GetValue() << endl;

                    pInt = sNodeMap.GetNode("StreamFailedBufferCount");
                    if (!IsAvailable(pInt) || !IsReadable(pInt))
                    {
                        cout << "Unable to read StreamFailedBufferCount. Aborting..." << endl << endl;
                        return -1;
                    }
                    cout << "StreamFailedBufferCount:" << pInt->GetValue() << endl;

                    pInt = sNodeMap.GetNode("StreamInputBufferCount");
                    if (!IsAvailable(pInt) || !IsReadable(pInt))
                    {
                        cout << "Unable to read SStreamInputBufferCount. Aborting..." << endl << endl;
                        return -1;
                    }
                    cout << "StreamInputBufferCount:" << pInt->GetValue() << endl;

                    pInt = sNodeMap.GetNode("StreamOutputBufferCount");
                    if (!IsAvailable(pInt) || !IsReadable(pInt))
                    {
                        cout << "Unable to read StreamOutputBufferCount. Aborting..." << endl << endl;
                        return -1;
                    }
                    cout << "StreamOutputBufferCount:" << pInt->GetValue() << endl;

                    CEnumerationPtr pEnum = sNodeMap.GetNode("StreamType");
                    if (!IsAvailable(pEnum) || !IsReadable(pEnum))
                    {
                        cout << "Unable to read StreamType. Aborting..." << endl << endl;
                        return -1;
                    }
                    if (pEnum->GetIntValue() == StreamType_GigEVision) {
                        pInt = sNodeMap.GetNode("GevTotalPacketCount");
                        if (!IsAvailable(pInt) || !IsReadable(pInt))
                        {
                            cout << "Unable to read GevTotalPacketCount. Aborting..." << endl << endl;
                            return -1;
                        }
                        cout << "GevTotalPacketCount:" << pInt->GetValue() << endl;

                        pInt = sNodeMap.GetNode("GevFailedPacketCount");
                        if (!IsAvailable(pInt) || !IsReadable(pInt))
                        {
                            cout << "Unable to read GevFailedPacketCount. Aborting..." << endl << endl;
                            return -1;
                        }
                        cout << "GevFailedPacketCount:" << pInt->GetValue() << endl;

                        pInt = sNodeMap.GetNode("GevResendPacketCount");
                        if (!IsAvailable(pInt) || !IsReadable(pInt))
                        {
                            cout << "Unable to read GevResendPacketCount. Aborting..." << endl << endl;
                            return -1;
                        }
                        cout << "GevResendPacketCount:" << pInt->GetValue() << endl;
                    }
